...
switch ($lnk[1]) {
	case '':
		...
		$page_fucking_title = _("Админка NVR: разбор жалоб");
		include Base::PathTPL("header");
		include Base::PathTPL("left_side");
		while ($report = $db->fetch_array($query))
			include Base::PathTPL("nvr/admin/reports_list_report");
	...
	default:
		...
		$page_title = _("Админка NVR: разбор жалоб на игрока ") . $pl->take_steam_info('nickname');
		include Base::PathTPL("header");
		include Base::PathTPL("left_side");
		include Base::PathTPL("nvr/admin/reports_rate_info");
		if ($db->num_rows($query)) {
			include Base::PathTPL("nvr/admin/reports_rate_header");
			while ($report = $db->fetch_array($query))
				include Base::PathTPL("nvr/admin/reports_rate_report");
			include Base::PathTPL("nvr/admin/reports_rate_footer");
		} else include Base::PathTPL("nvr/admin/reports_rate_none");
		include Base::PathTPL("right_side");
		include Base::PathTPL("footer");
		break;
}
